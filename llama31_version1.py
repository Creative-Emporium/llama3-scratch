...
#NOTE: Latest changes from NON-PUSHED version of llama3.1 to Andrej's version
...
class Llama:
    ...
    def chat_completion(
        ...
        if max_gen_len is None:
            max_gen_len = self.model.params.max_seq_len - 1
        #first format the prompts 
        prompt_tokens = [self.formatter.encode_dialog_prompt(d) for d in dialogues]
        generation_tokens = self.generate(
            prompt_tokens=prompt_tokens,
            sample_rng=sample_rng,
            max_gen_len=max_gen_len,
        ...
        responses = [{"generation": self.formatter.decode_response(t)} for t in generation_tokens]
        # responses = [
        #     {
        #         "generation": {
        #             "role": "assistant",
...
#----------------------training loop-------------------------
    ...
    print(f"hi Im the {ddp_rank} th GPU")
    ...
    for step in range(max_steps):
        ...
        if master_process:
            print(f"step {step}| loss: {loss.item():.4f}| tok/sec: {tokens_per_sec:.2f}| GPU:{ddp_local_rank}")
    if ddp:
...
# ----------------------Training loop for Chat/assitant model-------------------------

# ----------------------Inference Chat/assitant model-------------------------
def main(
    ckpt_dir: str,
    tokenizer_path: str,
    temperature: float = 0.6,
